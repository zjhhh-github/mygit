...
class DatabaseManagerApp:
    ...
    def create_widgets(self):
        ...
        self.host_var = tk.StringVar(value="100.66.1.1")
        ...
        self.port_var = tk.StringVar(value="13306")
        ...
        self.database_var = tk.StringVar(value="Student")
        ...
        # 右侧显示区域
        self.display_area = ScrolledText(right_frame, wrap=tk.WORD, width=70, height=15)
    ...
    def create_table(self):
        ...
        columns_text = ScrolledText(create_window, height=15)
    ...
    def table_operations_window(self, table_name):
        ...
        # 显示区域
        self.current_display_widget = ScrolledText(op_window, wrap=tk.WORD)
    ...
    def view_records(self, table_name):
        ...
        try:
            ...
            messagebox.showerror("错误", f"查询记录失败: {str(e)}")
        ...
        try:
            ...
            def submit_insert():
                ...
                try:
                    ...
                    messagebox.showerror("错误", f"插入记录失败: {str(e)}")
    ...
    def paste_insert_record_gui(self, table_name):
        ...
        try:
            ...
            # 文本输入区域
            text_area = ScrolledText(paste_window, height=10)
            ...
            structure_text = ScrolledText(paste_window, height=6)
            ...
            def submit_paste_insert():
                ...
                if len(parsed_values) > len(columns):
                    parsed_values = parsed_values[:len(columns)]
                ...
                try:
                    ...
                    for i, (col_info, value) in enumerate(zip(columns, parsed_values)):
                        col_name = col_info[0]
                        extra_info = col_info[5] if len(col_info) > 5 else ''
                        
                        # 跳过自增列
                        if 'auto_increment' in extra_info.lower():
                            continue
                            
                        filtered_cols.append(f"`{col_name}`")
                        filtered_values.append(value if value else None)
                    
                    if len(filtered_cols) == 0:
                        messagebox.showwarning("警告", "没有可插入的列（可能是自增列）")
                        return
                    ...
                    messagebox.showinfo("成功", f"成功插入记录！影响行数: 1")
                    paste_window.destroy()
                except Exception as e:
                    messagebox.showerror("错误", f"插入记录失败: {str(e)}")
            
            ttk.Button(paste_window, text="提交", command=submit_paste_insert).pack(pady=10)
    ...
    def execute_sql_window(self):
        ...
        def execute_sql():
            ...
            try:
                ...
                result_text.delete(1.0, tk.END)
                result_text.insert(tk.END, f"执行失败: {str(e)}")
