    ...
    try:
        ...
        while True:
            ...
            if dialog_hwnd:
                ...
                # 在最大等待时间内持续查找OK按钮
                while wait_time < max_wait_time and not ok_button_found:
                    time.sleep(1)
                    wait_time += 1
                    # 在弹窗中查找OK按钮
                    ok_button_hwnd = find_ok_button_in_dialog(dialog_hwnd)
                    if ok_button_hwnd:
                        print(f"在第 {wait_time} 秒找到了OK按钮")
                        ok_button_found = True
                    else:
                        print(f"第 {wait_time} 秒: 未找到OK按钮，继续等待...")
                ...
                if ok_button_found:
                    ...
                    print(f"在 {max_wait_time} 秒内未找到OK按钮，尝试其他方法...")
                    # 如果长时间未找到OK按钮，也尝试关闭程序
                    subprocess.run(['taskkill', '/f', '/im', 'chatlog.exe'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                    print("已终止chatlog.exe进程")
                    return
...
class ChatLogApp:
    def __init__(self, root):
        ...
        self.exit_button.grid(row=0, column=7, padx=2, sticky="e")
